package http

import (
	"testing"

	"github.com/smiecj/go_common/util/log"
)

func TestMakeGetRequest(t *testing.T) {
	client := GetHTTPClient()
	rsp, err := client.Do(Get(), Url("https://prometheus.io/docs/introduction/overview/"))
	if nil != err {
		log.Error("[TestMakeGetRequest] request failed: %s", err.Error())
		t.FailNow()
	}

	log.Info("[TestMakeGetRequest] request success: %s", rsp.RspBody)
}

/* func TestMakePostUrlEncodeRequest(t *testing.T) {
	client := GetHTTPClient()
	rsp, err := client.Do(Get(), Url("http://azkaban_address/userReq/doLogin"),
		AddParam("loginEmail", "admin"), AddParam("password", "admin123"))
	if nil != err {
		log.Error("[TestMakePostUrlEncodeRequest] request failed: %s", err.Error())
		t.FailNow()
	}

	log.Info("[TestMakePostUrlEncodeRequest] request success: body: %s", rsp.RspBody)
	for key, value := range rsp.RspHeader {
		log.Info("[TestMakePostUrlEncodeRequest] rsp header: %s -> %s", key, value)
	}
}
*/
